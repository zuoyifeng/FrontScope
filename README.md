...
FrontScope is being built around three long-term directions:

1. **Authenticated page diagnostics**: reliably scan protected routes in local and production web apps without accidentally measuring a login page.
2. **Framework-neutral frontend health checks**: support React, Vue, Angular, Next.js, Nuxt, Solid, plain JavaScript/TypeScript, and common Vite/Webpack projects through adapter-based scanners.
3. **Zero-config local project mode**: accept a local project folder, infer how to install/run it in a sandbox, discover important routes, and ask the user which routes should be monitored.

Detailed future work is tracked in [docs/frontscope-future-roadmap.md](docs/frontscope-future-roadmap.md). Contributor-sized implementation steps are tracked in [docs/frontscope-roadmap-implementation-plan.md](docs/frontscope-roadmap-implementation-plan.md).
...
当前限制：使用 `--auth-state` 时，运行时、Network、Performance Trace 和 Memory 会复用登录态；Lighthouse 也会通过 Playwright 注入 `storageState` 后采集 LCP/CLS/TBT 等指标。若 Lighthouse 在持续轮询的大屏页上失败，Performance Trace 仍会提供 Long Task 与 Layout Shift 近似指标。

更大的限制是：如果一个 Web 项目的关键业务页面都在权限路由后面，FrontScope 目前仍需要用户手动提供目标 URL 和登录态。未来会引入 Auth Profile 增强、路由发现和认证态路由验证，让本地与线上模式都能更可靠地发现并监测受保护页面。
...
- ⚠️ 权限路由覆盖：当前可以复用登录态扫描指定 URL，但还不能自动发现受保护业务路由全集
...
- Online mode can reuse a Playwright `storageState` file.
- Target mismatch detection warns when the browser lands on a login or unauthorized page.
- Users still need to provide the exact protected URL manually.

Planned work:

- Auth Profile manager: validate, refresh, and expire saved login states.
...
- Route verification: distinguish target page, login page, unauthorized page, blank shell, and redirect loops.
