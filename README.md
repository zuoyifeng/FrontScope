...
> 项目状态：早期开源阶段。核心扫描和报告功能已可用，但登录态路由覆盖、跨框架静态分析和零配置本地项目执行仍在路线图中。
...
- 线上模式可通过 UI 可视化录制或 CLI `--auth-state` 复用 Playwright `storageState` 文件。
- 可视化录制器打开真实浏览器供用户手动登录（验证码/MFA/SSO），然后在保存前验证目标 URL。
- 已有配置可针对当前扫描 URL 重新验证。
- Lighthouse 使用相同登录态运行，采集受保护页面的 LCP/CLS/TBT/Speed Index。
- 目标不匹配检测会在浏览器落到登录页或未授权页面时发出警告。
- 用户仍需手动提供每个受保护的 URL。

计划工作：

- 登录态配置生命周期：刷新计划、过期警告和批量重新验证。
- 登录态路由发现：扫描路由清单、站点地图文件、框架路由文件夹和登录后的运行时导航链接。
- 大规模路由验证：区分目标页面、登录页面、未授权页面、空白壳和重定向循环。
- 监控集合：将受保护路由组保存为可重复扫描的目标。

### 2. 跨框架项目质量

当前状态：

- 运行时、Network、Trace、Lighthouse、Memory 和截图证据与框架无关。
- 包检测可识别多个框架特征。
- 本地 AST 审查规则对 React/TypeScript 项目支持最强。

计划适配器：

- React / Next.js：JSX 规则、路由提取、打包和 hydration 提示。
- Vue / Nuxt：SFC 解析、模板规则、路由提取、Pinia/Vue Router 提示。
- Angular：工作区检测、路由配置解析、模板检查。
- Solid：适配 Solid 原语和路由模式的 JSX 规则。
- 纯 JavaScript 项目：为无 TypeScript 项目提供 Babel/Acorn 解析路径。

### 3. 零配置本地项目模式

当前状态：

- 本地模式需要 `projectPath` 和已启动的服务 URL。
- 项目质量检查针对提供的本地文件夹运行。
